...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "pegasus capture A01.1";
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        // Store connection info for data sending
        is_connected = true;
...
{
    if (red_samples == NULL || ir_samples == NULL || num_samples == 0 || sample_rate_hz <= 0) {
        return ESP_ERR_INVALID_ARG;
    }
    
    if (!is_connected || gatts_if_connected == ESP_GATT_IF_NONE || char_handle_connected == 0) {
        // Not connected, silently fail
        return ESP_ERR_INVALID_STATE;
    }
    
    // Get timestamp of first sample (microseconds since boot)
    int64_t timestamp_us = esp_timer_get_time();
    
    // Calculate sample interval in microseconds
    uint32_t sample_interval_us = (uint32_t)(1000000.0f / sample_rate_hz);
    
    // Calculate how many samples we can fit in one packet
    // Each sample: 4 bytes (red) + 4 bytes (IR) = 8 bytes
    // Header: 1 byte (sample count) + 8 bytes (timestamp) + 4 bytes (sample interval) = 13 bytes
    // Max packet size: local_mtu - 3 (ATT overhead) = typically 20-500 bytes
    // For safety, use min(MTU-3, 250) to leave room
    uint16_t max_packet_size = (local_mtu > 3) ? (local_mtu - 3) : 20;
    if (max_packet_size > 250) {
        max_packet_size = 250; // Cap at reasonable size
    }
    
    // Calculate samples per packet: (max_packet_size - 13) / 8
    uint8_t samples_per_packet = (max_packet_size > 13) ? ((max_packet_size - 13) / 8) : 0;
    if (samples_per_packet == 0) {
        samples_per_packet = 1; // At least 1 sample
    }
    
    // Send data in chunks
    uint8_t offset = 0;
    while (offset < num_samples) {
        uint8_t samples_in_this_packet = (num_samples - offset > samples_per_packet) ? 
                                         samples_per_packet : (num_samples - offset);
        
        // Calculate timestamp for first sample in this packet
        int64_t packet_timestamp_us = timestamp_us + (offset * sample_interval_us);
        
        // Allocate buffer: 1 byte (sample count) + 8 bytes (timestamp) + 4 bytes (interval) + samples * 8 bytes
        uint16_t packet_size = 1 + 8 + 4 + (samples_in_this_packet * 8);
        uint8_t *packet = (uint8_t *)malloc(packet_size);
        if (packet == NULL) {
            ESP_LOGE(GATTS_TAG, "Failed to allocate memory for sensor data packet");
            return ESP_ERR_NO_MEM;
        }
        
        // Packet format:
        // Byte 0: Number of samples in this packet
        // Bytes 1-8: Timestamp of first sample (64-bit microseconds since boot, little-endian)
        // Bytes 9-12: Sample interval in microseconds (32-bit, little-endian)
        // Bytes 13-16: Red sample 0 (32-bit, little-endian)
        // Bytes 17-20: IR sample 0 (32-bit, little-endian)
        // Bytes 21-24: Red sample 1 (32-bit, little-endian)
        // Bytes 25-28: IR sample 1 (32-bit, little-endian)
        // ... and so on
        
        packet[0] = samples_in_this_packet;
        
        // Timestamp (64-bit, little-endian)
        packet[1] = (packet_timestamp_us >> 0) & 0xFF;
        packet[2] = (packet_timestamp_us >> 8) & 0xFF;
        packet[3] = (packet_timestamp_us >> 16) & 0xFF;
        packet[4] = (packet_timestamp_us >> 24) & 0xFF;
        packet[5] = (packet_timestamp_us >> 32) & 0xFF;
        packet[6] = (packet_timestamp_us >> 40) & 0xFF;
        packet[7] = (packet_timestamp_us >> 48) & 0xFF;
        packet[8] = (packet_timestamp_us >> 56) & 0xFF;
        
        // Sample interval (32-bit, little-endian)
        packet[9] = (sample_interval_us >> 0) & 0xFF;
        packet[10] = (sample_interval_us >> 8) & 0xFF;
        packet[11] = (sample_interval_us >> 16) & 0xFF;
        packet[12] = (sample_interval_us >> 24) & 0xFF;
        
        // Samples
        for (uint8_t i = 0; i < samples_in_this_packet; i++) {
            uint32_t red = red_samples[offset + i];
            uint32_t ir = ir_samples[offset + i];
            
            uint16_t base_offset = 13 + (i * 8);
            
            // Red sample (little-endian)
            packet[base_offset + 0] = (red >> 0) & 0xFF;
            packet[base_offset + 1] = (red >> 8) & 0xFF;
            packet[base_offset + 2] = (red >> 16) & 0xFF;
            packet[base_offset + 3] = (red >> 24) & 0xFF;
            
            // IR sample (little-endian)
            packet[base_offset + 4] = (ir >> 0) & 0xFF;
            packet[base_offset + 5] = (ir >> 8) & 0xFF;
            packet[base_offset + 6] = (ir >> 16) & 0xFF;
            packet[base_offset + 7] = (ir >> 24) & 0xFF;
        }
        
        // Send notification
        esp_err_t ret = esp_ble_gatts_send_indicate(gatts_if_connected, conn_id_connected,
                                                   char_handle_connected, packet_size,
                                                   packet, false);
        
        free(packet);
        
        if (ret != ESP_OK) {
            ESP_LOGE(GATTS_TAG, "Failed to send sensor data: %s", esp_err_to_name(ret));
            return ret;
        }
        
        offset += samples_in_this_packet;
        
        // Small delay between packets to avoid overwhelming the stack
